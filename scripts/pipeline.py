...
def img_load(data_path, filename, dataset):
    ...
    if dataset == 'ade20k':
        ...
        #img = corrupt(img, corruption_name='saturate', severity=1)
        #img = mmcv.imresize(img, (256, 320))
    else:
...
def semantic_segment_anything_inference(filename, output_path, rank, img=None, save_img=False,
    ...
    if model == 'oneformer':
        class_ids = oneformer_func[dataset](Image.fromarray(img), semantic_branch_processor,
                                                                        semantic_branch_model, rank, anns)
    elif model == 'segformer':
        class_ids = segformer_func(img, semantic_branch_processor, semantic_branch_model, rank, anns)
    else:
        raise NotImplementedError()
